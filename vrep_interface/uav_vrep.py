...
def follow_path(clientID,path,goal):
    pathx=path[0]
    pathy=path[1]
    pathz=path[2]
    ...
    pangle=0
    pref_angz=0
    while (xPosition > pathx[-1]+0.1) or (xPosition < pathx[-1]-0.1) or (yPosition > pathy[-1]+0.1) or (yPosition < pathy[-1]-0.1) or (zPosition > pathz[-1]+0.1) or (zPosition < pathz[-1]-0.1):
        xvelomax=0.6
        yvelomax=0.6
        zmax=1
        absolut_dis=math.sqrt((xPosition-pathx[-1])**2+(yPosition-pathy[-1])**2+(zPosition-pathz[-1])**2)
        #print absolut_dis
        slowvelo_dis=4
        ...
        # Find nearest point
        pnear,_ =  pathfollowing.find_nearp(pos, path)

        # Calculate velocity reference
        vec= pathfollowing.calc_vel_ref(pos,path)
        ...
        xvelo_w=xvelomax*vec[0]/absolut
        yvelo_w=yvelomax*vec[1]/absolut
        height = pnear[2]

        # Now angular reference
...
#            print (pveloz,veloz)
        ...
        #Convert from world coordinates to UAV coordinate system
        ...
        #Send control packet to VREP
        control_data=[xvelo,yvelo,height,0,0,veloz]
        packedData=vrep.simxPackFloats(control_data)
